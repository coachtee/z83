...
| PATCH | `/applications/:id/status` | `{ status }`. `closed` is always allowed. `submitted` is only reachable manually from `print_prepared` — the applicant self-reporting "I hand-delivered this." From `email_prepared` it's rejected (`INVALID_TRANSITION`): only a genuinely dispatched `/send` can move an email-track application to `submitted`, so that status always has a matching `email_deliveries` row behind it. |
